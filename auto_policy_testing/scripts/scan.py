            ...
            try:
                ...
                if policy_resource.startswith(f'aws.{resource}') or policy_resource.startswith(resource):
                    command = (f"custodian run {os.path.join(POLICY_DIR, policy)} --dry-run --output-dir={os.path.join(OUTPUT_DIR, region)} \
                               ...
                               {f'--assume {sa}'  if sa and CLOUD == 'GCP' else ''} {f'--assume {sa}'  if sa and CLOUD == 'AWS' else ''}")
                    process = subprocess.Popen([command], shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
                    process.wait()
                    policy_resource = policy_resource.split('.')[1] if "." in policy_resource else policy_resource
                    name_prefix = output(path, policy_name=policy.split('.')[0], resource=policy_resource)
                    policy_execution_outputs[policy.split('.')[0]] = {'scan_result': process.stdout.read().decode('utf-8'), 'name': name_prefix, 'policy_resource': policy_resource}
                    if region != "default":
