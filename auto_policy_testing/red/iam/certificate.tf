resource "null_resource" "this" {
  provisioner "local-exec" {
    command     = "openssl req -x509 -nodes -days 2 -newkey rsa:2048 -keyout private.key -out certificate.crt -subj '/C=US/ST=State/L=City/O=Organization/OU=Department/CN=example.com'"
