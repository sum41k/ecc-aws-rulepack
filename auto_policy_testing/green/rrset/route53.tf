...
resource "aws_route53_record" "this2" {
  ...
  name            = aws_route53_zone.this.name
  type            = "AAAA"
  set_identifier  = "${module.naming.resource_prefix.r53record}-2"
...
resource "aws_route53_record" "this3" {
  ...
  name            = aws_route53_zone.this.name
  type            = "MX"
  set_identifier  = "${module.naming.resource_prefix.r53record}-3"
  health_check_id = aws_route53_health_check.this.id
  ttl             = 300
...
resource "aws_route53_health_check" "this" {
  reference_name    = "autotest_health_check_green"
  ip_address        = "1.1.1.1"
  port              = 80
  type              = "HTTP"
  resource_path     = "/"
